...
//   $json = @file_get_contents("https://www.amcharts.com/tools/country/?v=xz6Z");
//   if ($json === false) {
...
<?php include "./includes/sidebar.php"; ?>
    ...
    <main class="ml-72 w-full p-6 overflow-y-auto">
        <h1 class="text-2xl font-bold mb-4">Welcome, <?= htmlspecialchars($user_name) ?>!</h1>

    <div class="province-selector border p-2 px-3 bg-white my-2">
        <div class="text-xl font-semibold">Select a province</div>
        <div class="text-sm text-gray-500">Choose the province of application below.</div>

        <select name="" id="province-selector" class=" w-full p-2 border cursor-pointer ring-blue-500">
        <?php
  ...
  <!-- <div class="text-md font-semibold ">You can use the map by selecting the province of application.</div> -->
  <div id="chartdiv" class=" cursor-pointer"></div>


<div id="province-modal" class=" hidden md:w-[70vw] sm:w-[90vw] absolute p-4 flex justify-center">

  <div class=" province-container bg-white rounded-md">
    <div id="province-img" class="province-img p-4 px-8 py-20 rounded-md rounded-b">
      <div class="flex justify-between">

        <div class="province-name">
          <div class="header-text text-2xl text-white font-medium"><span id="province-name-span1"></span> Province</div>
        </div>
      ...
      <div class=" text-white text-md">
          The <span id="province-name-span2"></span> province immigration offices.
      </div>
...
let provinceSelector = document.getElementById("province-selector");
let provinceImg = document.getElementById("province-img");
...
// load data 
    ...
    $.ajax({
      ...
      success: (response) => {
        ...
        if (offices.length === 0) {
          ...
          let officeHTML = `
          <a href="application_office.php?id=${office.id}">
          <div class="office w-full sm:w-[48%] md:w-[45%] lg:w-[45%] p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-green-400 hover:shadow-md transition-all duration-200 cursor-pointer">
  ...
  provinceSelector.addEventListener("change", ()=>{
      loadData(provinceSelector.value);
      provinceModal.style.display = "block";
      zambianMap.style.display = "none";

      let provinceName = provinceSelector.value;
      const fileName = provinceName.trim().toLowerCase() + '.jpg';
      provinceImg.style.backgroundImage = `url("./provinces/${fileName}")`;

      provinceNameSpan1.innerHTML = provinceSelector.value;
      provinceNameSpan2.innerHTML = provinceSelector.value;
...
//   am5.net.load("apply.php?proxy=country", chart).then(function (result) {
//     var geo = am5.JSONParser.parse(result.response);
//     console.log("geo content:", geo);
